<template>
  <div class="dmode">
    <VueGroup v-model="mode">
      ...
      <VueGroupButton class="big" value="view" icon-left="airplay">Viewer</VueGroupButton>
    </VueGroup>
    <form class="dmode__form" @submit.prevent="join">
      <VueFormField>
        <VueInput v-model="txtHost" placeholder="Table name" required />
      </VueFormField>
      <VueButton type="submit" class="primary dmode__btn">Join</VueButton>
    </form>
  </div>
</template>
<script lang="ts">
...
export default Vue.extend({
  ...
  methods: {
    join() {
      this.$router.replace(`/${this.mode}/${this.txtHost}`);
    }
  }
});
</script>
