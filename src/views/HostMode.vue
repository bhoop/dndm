    ...
    <div class="host__assets">
      <template v-for="group in assetGroups">
        <div class="host__heading" v-if="group.title">
          {{ group.title }}
          <VueButton
            class="info icon-button flat round add-asset-button"
            :loading="pendingCreate===group.type"
            icon-left="add"
            @click="createAsset(group.type)"
          />
        </div>
        <AssetThumbnail
...
export default Vue.extend({
  ...
  computed: {
    ...
    openAsset() {
      return this.$store.state.assets.find(
        r => r.id === this.$route.params.asset
      );
    },
    assetGroups() {
  ...
  methods: {
    displayAsset(which) {
      ...
      if (old && old.id === which.id)
        return db.doc(`assets/${which.id}`).update({ display: false });
      db.doc(`assets/${which.id}`).update({ display: true });
      if (old)
        db.doc(`assets/${this.activeAsset.id}`).update({ display: false });
    },
    editAsset(which) {
      clearTimeout(this.openTimer);
      this.openTimer = setTimeout(() => {
        this.$router.replace(
          `/host/${this.$route.params.table}/asset/${which.id}`
        );
      }, 300);
    },
    updateAsset(id, data) {
      if (data.display) {
        ...
        if (existing && existing.id !== id)
          db.doc(`assets/${existing.id}`).update({ display: false });
      }
      db.doc(`assets/${id}`).update(data);
    },
    deleteAsset(id) {
      if (this.$route.params.asset === id)
    ...
    async createAsset(type) {
      ...
      switch (type) {
        case "encounter":
          ...
          data.background = this.$store.state.assets.find(
            r => r.type === "env"
          ).picture;
          data.characters = this.$store.state.assets
            .filter(r => r.type === "pc")
            .map(r => ({
              uid: nanoid(),
              active: false,
